...
async def get_current_user(
    ...
    try:
        ...
        raise credentials_exception
    user_dict = await crud.find_existed_user(token_data.email, session)
    if user_dict is None:
        raise credentials_exception

    return UserObjectSchema(**user_dict)
...
def get_current_active_user(
    ...
    if hasattr(current_user, 'user_role') and current_user.user_role == 'disabled':
        raise HTTPException(status_code=400, detail="Inactive user")

    if hasattr(current_user, 'user_status') and current_user.user_status == 0:
        raise HTTPException(status_code=400, detail="Inactive user")
    return current_user